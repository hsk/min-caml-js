...
let rec to_if e = match e with
  ...
  | Array (e1, e2) -> App(Var "makeArray", [to_if e1; to_if e2]) 
...
let rec alpha env = function
  ...
  | Unit -> Unit
  | Bin(e1,op,e2) -> Bin(alpha env e1,op, alpha env e2)
  | Pre(op,e1) -> Pre(op, alpha env e1)
  | App(e,es) -> App(alpha env e, List.map (alpha env) es)
  | If(e1,e2,e3) -> If(alpha env e1, alpha env e2, alpha env e3)
  | Let(s,e1,e2) ->
    let s' = if mem env s then genid s else s in
    Let(s',alpha env e1, alpha ((s,s')::env) e2)
  | LetRec(s, e1, e2) ->
    let s' = if mem env s then genid s else s in
    LetRec(s', alpha ((s,s')::env) e1, alpha ((s,s')::env) e2)
  | Fun(is,e) ->
    let iis = List.map(fun i -> (i,i)) is in
    Fun(is, alpha (iis @ env) e)  
  | Rec(ies) ->
    Rec(List.map(fun(i,e)->(i,alpha env e)) ies)
  | Get(e1,e2) -> Get(alpha env e1, alpha env e2)
  | Put(e1,e2,e3) -> Put(alpha env e1, alpha env e2, alpha env e3)
